...
public class DatabasePhoneNumber extends SQLiteOpenHelper {
    ...
    public ArrayList<com.example.ourcafeee.UserPhoneNumber> getAppCategoryDetail() {

        final String TABLE_NAME = com.example.ourcafeee.DatabasePhoneNumber.TABLE_NAME;

        String selectQuery = "SELECT  * FROM " + TABLE_NAME;
        SQLiteDatabase db  = this.getReadableDatabase();
        Cursor cursor      = db.rawQuery(selectQuery, null);
        ArrayList<com.example.ourcafeee.UserPhoneNumber> data      = new ArrayList<>();

        if (cursor.moveToFirst()) {
            do {
                com.example.ourcafeee.UserPhoneNumber information=new com.example.ourcafeee.UserPhoneNumber();
