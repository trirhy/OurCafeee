...
public class DatabaseEmailId extends SQLiteOpenHelper {
    ...
    public ArrayList<com.example.ourcafeee.UserEmailId> getAppCategoryDetail() {

        final String TABLE_NAME = com.example.ourcafeee.DatabaseEmailId.TABLE_NAME;

        String selectQuery = "SELECT  * FROM " + TABLE_NAME;
        SQLiteDatabase db  = this.getReadableDatabase();
        Cursor cursor      = db.rawQuery(selectQuery, null);
        ArrayList<com.example.ourcafeee.UserEmailId> data      = new ArrayList<>();

        if (cursor.moveToFirst()) {
            do {
                com.example.ourcafeee.UserEmailId information=new com.example.ourcafeee.UserEmailId();
