...
public class DatabaseAddress extends SQLiteOpenHelper {
    ...
    public ArrayList<com.example.ourcafeee.UserAddress> getAppCategoryDetail() {

        final String TABLE_NAME = com.example.ourcafeee.DatabaseAddress.TABLE_NAME;

        String selectQuery = "SELECT  * FROM " + TABLE_NAME;
        SQLiteDatabase db  = this.getReadableDatabase();
        Cursor cursor      = db.rawQuery(selectQuery, null);
        ArrayList<com.example.ourcafeee.UserAddress> data      = new ArrayList<>();

        if (cursor.moveToFirst()) {
            do {
                com.example.ourcafeee.UserAddress information=new com.example.ourcafeee.UserAddress();
