...
public class DatabaseNames extends SQLiteOpenHelper {
    ...
    public ArrayList<com.example.ourcafeee.UserName> getAppCategoryDetail() {

        final String TABLE_NAME = com.example.ourcafeee.DatabaseNames.TABLE_NAME;

        String selectQuery = "SELECT  * FROM " + TABLE_NAME;
        SQLiteDatabase db  = this.getReadableDatabase();
        Cursor cursor      = db.rawQuery(selectQuery, null);
        ArrayList<com.example.ourcafeee.UserName> data      = new ArrayList<>();

        if (cursor.moveToFirst()) {
            do {
                com.example.ourcafeee.UserName information=new com.example.ourcafeee.UserName();
